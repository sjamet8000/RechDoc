*en cours de rédaction*

## Comment l'utiliser en l'état

### Accéder à la ressource

Le lien d'accès à la formation est le suivant :
https://liascript.github.io/course/?https://raw.githubusercontent.com/sjamet8000/RechDoc/refs/heads/main/FmoduleDroit.md#1

### Comprendre l'interface de Liascript

Blablabbal test ???

Une ressource Liascript est présentée comme un diaporama interactif :

- à **gauche** : un menu escamotable listant les chapitres et sous-parties

- au **centre** : la diapositive actuelle, contenant le texte, les images, les activités, etc.

- en **haut à droite** : une série de boutons pour :

  - modifier le mode de présentation

  - modifier les couleurs de base de l'interface, la taille du texte, etc.

  - modifier la langue si une traduction est disponible

  - partager la ressource avec un QR Code

  - ouvrir l'onglet "informations" de la ressource

Naviguer entre les sections
==========================

**Méthode 1**

- Placez votre curseur à gauche de l’écran.

- Cliquez sur le chapitre que vous souhaitez lire.

- Cliquez sur une sous-partie pour l’afficher.


**Méthode 2**

En bas à gauche ou au centre de l’écran, vous pouvez cliquer :

- Flèche → pour aller à la diapositive suivante

- Flèche ← pour revenir en arrière

### Utiliser les éléments interactifs

Révélation de contenus successive
-----------------------------

**Uniquement** en mode Présentation et en mode Diapositives, certains paragraphes apparaissent **progressivement** lorsque vous cliquez.

Vous pouvez le voir sur l'interface de navigation de la ressource (les petites flèches) qui indiquent le n° de la slide et entre parenthèse le nombre de cliques à faire pour faire apparaitre tous le contenu :

Répondre à un quiz / question interactive
------------------------------------

Il peut s'agir de QCM, de questions ouvertes, etc.

Exemple : QCM
==============

1. Lisez la question.

2. Cliquez sur la réponse de votre choix.

3. Cliquez sur **Vérifier**

4. Un retour automatique apparaît (correct / incorrect).

5. Si cela a été intégré, un feedback spécifique à la question apparait

Exemple : QRC
=============

1. Lisez la question.

2. Entrez la réponse

3. Cliquez sur **Vérifier**

4. Un retour automatique apparaît (correct / incorrect).

5. Si cela a été intégré, un feedback spécifique à la question apparait

#### Vidéos intégrées

Certaines slides contiennent des vidéos. S'il s'agit d'intégration depuis Youtube, elles fonctionnent comme d'habitude.

#### Activités H5P intégrées

Certaines slides contiennent des activités H5P. Ces dernière peuvent mettre 1 ou deux secondes à s'afficher.

#### Autres intégrations

Les slides peuvent aussi contenir d'autres intégrations iframe (Genially par exemple)

![](assets\README\RM_integ_genially.png)


### Comprendre l'interface de Liascript
## Comment la maintenir et la sauvegarder
## Comment la modifier
### Modifier du texte
### Modifier des images
### Modifier du H5P
